...
def run(ctx):
    ...
    magdeck.disengage()
    magheight = MAG_OFFSET + magnets.height.by_serial.get(
        magdeck._module._driver.get_device_info()['serial'], 6.65)
...
resuming.')
    ...
    current_status = 'Removing Supernatant'
    remove_supernatant(STARTING_VOL)
    update_log_file()
    current_status = 'washing A'
    wash(WASH_VOL, washA, 20)
    update_log_file()
    current_status = 'washing B'
    wash(WASH_VOL, washB, 20)
    update_log_file()
    current_status = 'Station B part 1 DONE'
    magdeck.disengage()
    update_log_file()
    tip_track()
